...
static GdkGLContext *
gdk_gl_context_new_common (GdkGLConfig *glconfig, GdkGLContext *share_list, int render_type, GLXContext glxcontext, gboolean is_foreign)
{
  ...
  if (share_list && GDK_IS_GL_CONTEXT_IMPL_X11(share_list))
    {
...
_gdk_x11_gl_context_new (GdkGLDrawable *gldrawable, GdkGLContext *share_list, gboolean direct, int render_type)
{
  ...
  GdkGLConfig* glconfig = G_OBJECT_TYPE(gldrawable) == GDK_TYPE_GL_WINDOW
	? gdk_gl_window_get_gl_config (gldrawable)
	: gdk_gl_pixmap_get_gl_config (gldrawable);
  ...
  if (_gdk_gl_context_force_indirect) {
    ...
    glxcontext = glXCreateContext (xdisplay, xvinfo, share_glxcontext, False);
  } else {
    glxcontext = glXCreateContext (xdisplay, xvinfo, share_glxcontext, (direct == TRUE) ? True : False);
  }
  if (!glxcontext)
    return NULL;

  GDK_GL_NOTE (MISC, g_message (" -- Context: screen number = %d", xvinfo->screen));
  GDK_GL_NOTE (MISC, g_message (" -- Context: visual id = 0x%lx", xvinfo->visualid));

  /*
   * Instantiate the GdkGLContextImplX11 object.
   */

  return gdk_gl_context_new_common (glconfig, share_list, render_type, glxcontext, FALSE);
}
 ...
 **/
GdkGLContext *
gdk_x11_gl_context_foreign_new (GdkGLConfig *glconfig, GdkGLContext *share_list, GLXContext glxcontext)
{
  GDK_GL_NOTE_FUNC ();
 ...
 **/
gboolean
gdk_gl_context_copy (GdkGLContext *glcontext, GdkGLContext *src, unsigned long mask)
{
  ...
  glXCopyContext (GDK_GL_CONFIG_XDISPLAY (glconfig), src_glxcontext, dst_glxcontext, mask);
...
/*< private >*/
void
_gdk_gl_context_set_gl_drawable (GdkGLContext *glcontext, GdkGLDrawable *gldrawable)
{
  GdkGLContextImplX11 *impl = GDK_GL_CONTEXT_IMPL_X11 (glcontext);
  ...
  if (impl->gldrawable != NULL)
    {
      g_object_remove_weak_pointer (G_OBJECT(impl->gldrawable), (gpointer*) &(impl->gldrawable));
      impl->gldrawable = NULL;
    }
  ...
  if (gldrawable != NULL && GDK_IS_GL_DRAWABLE (gldrawable))
    {
      ...
      g_object_add_weak_pointer (G_OBJECT(impl->gldrawable), (gpointer*) &(impl->gldrawable));
    }
}
...
static void
gdk_gl_context_remove (GdkGLContextImplX11 *glcontext)
{
  GDK_GL_NOTE_FUNC_PRIVATE ();
  ...
  g_hash_table_remove (gl_context_ht, glcontext->glxcontext);
