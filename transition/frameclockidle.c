...
static gboolean
sleep_source_prepare (GSource *source, gint *timeout)
{
...
static gboolean
sleep_source_dispatch (GSource* source, GSourceFunc callback, gpointer user_data)
{
	return TRUE;
}
...
gdk_frame_clock_idle_init (GdkFrameClockIdle *frame_clock_idle)
{
	...
	frame_clock_idle->priv = G_TYPE_INSTANCE_GET_PRIVATE (frame_clock_idle, GDK_TYPE_FRAME_CLOCK_IDLE, GdkFrameClockIdlePrivate);
...
gdk_frame_clock_paint_idle (void *data)
{
	...
	gboolean skip_to_resume_events = (priv->requested & ~(GDK_FRAME_CLOCK_PHASE_FLUSH_EVENTS | GDK_FRAME_CLOCK_PHASE_RESUME_EVENTS)) == 0 && priv->updating_count == 0;
	...
	if (!skip_to_resume_events) {
		switch (priv->phase) {
			...
			case GDK_FRAME_CLOCK_PHASE_UPDATE:
				if (priv->freeze_count == 0) {
					if ((priv->requested & GDK_FRAME_CLOCK_PHASE_UPDATE) != 0 || priv->updating_count > 0) {
			...
			case GDK_FRAME_CLOCK_PHASE_LAYOUT:
				if (priv->freeze_count == 0) {
					...
					if ((_gdk_debug_flags & GDK_DEBUG_FRAMES) != 0) {
						if (priv->phase != GDK_FRAME_CLOCK_PHASE_LAYOUT && (priv->requested & GDK_FRAME_CLOCK_PHASE_LAYOUT))
...
#ifdef G_ENABLE_DEBUG
					if ((_gdk_debug_flags & GDK_DEBUG_FRAMES) != 0) {
						if (priv->phase != GDK_FRAME_CLOCK_PHASE_PAINT && (priv->requested & GDK_FRAME_CLOCK_PHASE_PAINT))
	...
	*/
	if (priv->freeze_count == 0) {
		priv->min_next_frame_time = compute_min_next_frame_time (clock_idle, priv->frame_time);
...
static void
gdk_frame_clock_idle_request_phase (GdkFrameClock* clock, GdkFrameClockPhase phase)
{
	GdkFrameClockIdle* clock_idle = GDK_FRAME_CLOCK_IDLE (clock);
	GdkFrameClockIdlePrivate *priv = clock_idle->priv;
