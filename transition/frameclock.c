...
static GdkFrameClock* default_clock = NULL;

void
frame_clock_request_phase(GdkFrameClockPhase phase)
{
	if(!default_clock){
		default_clock = g_object_new(GDK_TYPE_FRAME_CLOCK_IDLE, NULL);
	}

	gdk_frame_clock_request_phase (default_clock, phase);
}
...
{
	if(!default_clock) default_clock = g_object_new(GDK_TYPE_FRAME_CLOCK_IDLE, NULL);

	gdk_frame_clock_begin_updating(default_clock);
}
...
{
	gdk_frame_clock_end_updating(default_clock);
}


void
frame_clock_connect(GCallback callback, gpointer user_data)
{
	if(!default_clock){
		default_clock = g_object_new(GDK_TYPE_FRAME_CLOCK_IDLE, NULL);
	}
 ...
 */
void
gdk_frame_clock_request_phase (GdkFrameClock *frame_clock, GdkFrameClockPhase phase)
{
...
void
_gdk_frame_clock_debug_print_timings (GdkFrameClock *clock, GdkFrameTimings *timings)
{
	gint64 previous_frame_time = 0;
	GdkFrameTimings *previous_timings = gdk_frame_clock_get_timings (clock, timings->frame_counter - 1);
