...
static void
mock_load_blocks (Renderer* renderer, WaveformActor* actor, int b1, int b2)
{
	for (int b = b1; b <= b2; b++ ) {
		mock_load_block(renderer, actor, b);
	}
}

static bool
mock_pre_render (Renderer* renderer, WaveformActor* actor)
{
	return true;
}
...
MockRenderer mock_renderer_v_low = {{MODE_V_LOW, mock_low_new, mock_load_block, mock_load_blocks, mock_pre_render, mock_low_render_block, mock_low_post_render, mock_low_free_waveform}};
MockRenderer mock_renderer = {{MODE_LOW, mock_low_new, mock_load_block, mock_load_blocks, mock_pre_render, mock_low_render_block, mock_low_post_render, mock_low_free_waveform}};
MockRenderer mock_renderer_med = {{MODE_MED, mock_low_new, mock_load_block, mock_load_blocks, mock_pre_render, mock_low_render_block, mock_low_post_render, mock_low_free_waveform}};
MockRenderer mock_renderer_hi = {{MODE_HI, mock_low_new, mock_load_block, mock_load_blocks, mock_pre_render, mock_low_render_block, mock_low_post_render, mock_low_free_waveform}};
...
void queue_draw (AGlScene* a) {}
AGlScene scene = { .type = 4, .enable_animations = false, .queue_draw = queue_draw };
...
#if USE_EGL
	scene.gl.egl.draw_idle = 1; // prevent idle from running
#else
...
test_load_missing_blocks ()
{
	...
	_wf_actor_load_missing_blocks(wf_actor);
	assert(array_equal(vlow->load_blocks_calls, vlow->sp, NULL, 0), "%s: expected vlow blocks not called", stepname)
