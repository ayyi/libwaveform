...
ssize_t
ad_read_short_sndfile (WfDecoder* d, WfBuf16* buf)
{
...
static ssize_t
ad_read_s32_sndfile (WfDecoder* d, int32_t* out, size_t len)
{
	SndfileDecoder* sf = (SndfileDecoder*)d->d;
	...
	switch (d->info.bit_depth) {
		...
		case 16:
		case 24: {
			ssize_t r = sf_read_int(sf->sffile, out, len);
#ifdef DEBUG
			if (!r) printf("%s\n", sf_strerror(sf->sffile));
#endif
			return r / d->info.channels;
		}
#ifdef DEBUG
