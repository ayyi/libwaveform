...
pango_gl_font_get_face (PangoFont *font)
{
  ...
  if (!glfont->face)
	...
	goto bail0;
      ...
      error 
	= FT_New_Face (_pango_gl_font_map_get_library (fcfont->fontmap),
...
pango_gl_font_class_init (PangoGlFontClass *class)
{
  ...
  object_class->finalize        = pango_gl_font_finalize;
  
  font_class->get_glyph_extents = pango_gl_font_get_glyph_extents;
...
{
  PangoGlFont *font = (PangoGlFont *)font_;
  PangoFcFont *fcfont = (PangoFcFont *)font;
...
{
  PangoGlFont *font = PANGO_GL_FONT (data);
...
{
  PangoGlFont *font = (PangoGlFont *)object;
  ...
  g_hash_table_foreach_remove (font->glyph_info, pango_clutter_free_glyph_info_callback, object);
...
{
  PangoClutterGlyphInfo *info = pango_clutter_font_get_glyph_info (font, glyph_index, TRUE);
...
{
  PANGO_GL_FONT (font)->glyph_cache_destroy = destroy_notify;
}
