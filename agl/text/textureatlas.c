...
{
	GskGLTextureAtlases* self = g_new (GskGLTextureAtlases, 1);
...
#if 0
static void
write_atlas_to_png (GskGLTextureAtlas* atlas, const char* filename)
{
	...
	cairo_surface_t* s = cairo_image_surface_create_for_data (data, CAIRO_FORMAT_ARGB32, atlas->width, atlas->height, stride);
...
#if 0
void
gsk_gl_texture_atlases_begin_frame (GskGLTextureAtlases* self, GPtrArray* removed)
{
	for (int i = self->atlases->len - 1; i >= 0; i--) {
		GskGLTextureAtlas *atlas = g_ptr_array_index (self->atlases, i);

		if (gsk_gl_texture_atlas_get_unused_ratio (atlas) > MAX_OLD_RATIO) {
			dbg2(GLYPH_CACHE, 0, "Dropping atlas %d (%g.2%% old)", i, 100.0 * gsk_gl_texture_atlas_get_unused_ratio (atlas));
...
#if 0
  {
    ...
    timestamp++;
    if (timestamp % 10 == 0)
      for (int i = 0; i < self->atlases->len; i++) {
          GskGLTextureAtlas *atlas = g_ptr_array_index (self->atlases, i);
...
bool
gsk_gl_texture_atlases_pack (GskGLTextureAtlases* self, int width, int height, GskGLTextureAtlas** atlas_out, int* out_x, int* out_y)
{
	...
	if (atlas == NULL) {
		...
		dbg2(GLYPH_CACHE, 0, "adding new atlas");
	}
...
void
gsk_gl_texture_atlas_init (GskGLTextureAtlas* self, int width, int height)
{
	...
	*       nodes here? stb_rect_pack.h says with is optimal. */
	...
	stbrp_init_target (&self->context, width, height, self->nodes, width);
...
void
gsk_gl_texture_atlas_mark_unused (GskGLTextureAtlas* self, int width, int height)
{
...
void
gsk_gl_texture_atlas_mark_used (GskGLTextureAtlas* self, int width, int height)
{
	self->unused_pixels -= (width * height);
...
bool
gsk_gl_texture_atlas_pack (GskGLTextureAtlas *self, int width, int height, int* out_x, int* out_y)
{
	stbrp_rect rect;
 ...
 */
static guint
create_shared_texture (int width, int height)
{
	guint texture_id;
...
gsk_gl_texture_atlas_realize (GskGLTextureAtlas *atlas)
{
	...
	//gdk_gl_context_label_object_printf (gdk_gl_context_get_current (), GL_TEXTURE, atlas->texture_id, "Texture atlas %d", atlas->texture_id);
}
