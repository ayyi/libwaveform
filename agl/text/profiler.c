...
{
	self->counters = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL, named_counter_free);
	self->timers = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL, named_timer_free);
}
...
static NamedCounter*
named_counter_new (GQuark id, const char* description, gboolean can_reset)
{
...
static NamedCounter*
gsk_profiler_get_counter (GskProfiler* profiler, GQuark id)
{
...
GQuark
gsk_profiler_add_counter (GskProfiler* profiler, const char* counter_name, const char* description, gboolean can_reset)
{
...
static NamedTimer*
named_timer_new (GQuark id, const char* description, gboolean invert, gboolean can_reset)
{
...
static NamedTimer*
gsk_profiler_get_timer (GskProfiler* profiler, GQuark id)
{
...
GQuark
gsk_profiler_add_timer (GskProfiler* profiler, const char* timer_name, const char* description, gboolean invert, gboolean can_reset)
{
	NamedTimer* timer;
	GQuark id;
...
void
gsk_profiler_counter_inc (GskProfiler* profiler, GQuark counter_id)
{
...
void
gsk_profiler_counter_set (GskProfiler* profiler, GQuark counter_id, gint64 value)
{
	NamedCounter* counter;
	...
	if (counter == NULL) {
		g_critical ("No counter '%s' (id:%d) found; did you forget to call gsk_profiler_add_counter()?", g_quark_to_string (counter_id), counter_id);
		return;
	}
...
void
gsk_profiler_counter_add (GskProfiler* profiler, GQuark counter_id, gint64 increment)
{
	NamedCounter* counter;
...
void
gsk_profiler_timer_begin (GskProfiler* profiler, GQuark timer_id)
{
	NamedTimer* timer;
...
gint64
gsk_profiler_timer_end (GskProfiler* profiler, GQuark timer_id)
{
	NamedTimer* timer;
	gint64 diff;
	...
	timer = gsk_profiler_get_timer (profiler, timer_id);
	if (timer == NULL) {
		g_critical ("No timer '%s' (id:%d) found; did you forget to call gsk_profiler_add_timer()?", g_quark_to_string (timer_id), timer_id);
		return 0;
	}

	if (!timer->in_flight) {
		g_critical ("Timer '%s' (id:%d) is not running; did you forget to call gsk_profiler_timer_begin()?", g_quark_to_string (timer->id), timer->id);
		return 0;
	}
...
void
gsk_profiler_timer_set (GskProfiler* profiler, GQuark timer_id, gint64 value)
{
	NamedTimer* timer;
...
gint64
gsk_profiler_counter_get (GskProfiler* profiler, GQuark counter_id)
{
	NamedCounter* counter;
...
gint64
gsk_profiler_timer_get (GskProfiler* profiler, GQuark timer_id)
{
	NamedTimer* timer;
...
gint64
gsk_profiler_timer_get_start (GskProfiler* profiler, GQuark timer_id)
{
	NamedTimer* timer;
...
void
gsk_profiler_append_counters (GskProfiler* profiler, GString* buffer)
{
	GHashTableIter iter;
	gpointer value_p = NULL;
...
void
gsk_profiler_append_timers (GskProfiler* profiler, GString* buffer)
{
	GHashTableIter iter;
	gpointer value_p = NULL;
	int i;
	...
	while (g_hash_table_iter_next (&iter, NULL, &value_p)) {
		...
		g_string_append_printf (buffer, "%s (%s): %.2f", timer->description, unit, (double)timer->value / scale);

		if (timer->n_samples > 1) {
			timer->avg_value = timer->avg_value / timer->n_samples;
			g_string_append_printf (buffer, " Min: %.2f Avg: %.2f Max: %.2f (%" G_GINT64_FORMAT " samples)", (double)timer->min_value / scale, (double)timer->avg_value / scale,
			    (double)timer->max_value / scale, timer->n_samples);
		}
...
void
gsk_profiler_begin_gpu_region (GskProfiler* profiler)
{
	...
	GLuint query_id = profiler->gl_queries[profiler->active_query];
	glBeginQuery (GL_TIME_ELAPSED, query_id);
}


guint64
gsk_profiler_end_gpu_region (GskProfiler* profiler)
{
	GLuint64 elapsed;
...
bool
profiler_is_running (GskProfiler* profiler)
{
	return running;
}
...
void
profiler_add_mark (gint64 start, guint64 duration, const char* name, const char* message)
{
	if (!running)
		return;

	sp_capture_writer_add_mark (writer, start, -1, getpid (), duration, "gtk", name, message);
}
