...
static guint    glyph_cache_hash       (gconstpointer);
static gboolean glyph_cache_equal      (gconstpointer, gconstpointer);
static void     glyph_cache_key_free   (gpointer);
static void     glyph_cache_value_free (gpointer);


GskGLGlyphCache*
gsk_gl_glyph_cache_new (/*GdkDisplay* display, */GskGLTextureAtlases* atlases)
{
	GskGLGlyphCache* glyph_cache = g_new0 (GskGLGlyphCache, 1);

	glyph_cache->hash_table = g_hash_table_new_full (glyph_cache_hash, glyph_cache_equal, glyph_cache_key_free, glyph_cache_value_free);
...
static bool
render_glyph (GlyphCacheKey* key, GskGLCachedGlyph* value, GskImageRegion* region)
{
	cairo_scaled_font_t* scaled_font = pango_cairo_font_get_scaled_font ((PangoCairoFont *)key->data.font);
	if (G_UNLIKELY (!scaled_font || cairo_scaled_font_status (scaled_font) != CAIRO_STATUS_SUCCESS)) {
		g_warning ("Failed to get a font");
		return FALSE;
	}

	int surface_width = value->draw_width * key->data.scale / 1024;
	int surface_height = value->draw_height * key->data.scale / 1024;

	int stride = cairo_format_stride_for_width (CAIRO_FORMAT_ARGB32, surface_width);
	unsigned char* data = g_malloc0 (stride * surface_height);
	cairo_surface_t* surface = cairo_image_surface_create_for_data (data, CAIRO_FORMAT_ARGB32, surface_width, surface_height, stride);
	...
	PangoGlyphInfo glyph_info = {
		.glyph = key->data.glyph,
		.geometry = {
			.width = value->draw_width * 1024,
			.x_offset = (key->data.glyph & PANGO_GLYPH_UNKNOWN_FLAG)
				? 0
				: - value->draw_x * 1024,
			.y_offset = - value->draw_y * 1024
		}
	};
...
static void
upload_glyph (GlyphCacheKey* key, GskGLCachedGlyph* value)
{
	GskImageRegion r;

	//gdk_gl_context_push_debug_group_printf (gdk_gl_context_get_current (), "Uploading glyph %d", key->data.glyph);

	if (render_glyph (key, value, &r)) {
		glPixelStorei (GL_UNPACK_ROW_LENGTH, r.stride / 4);
		glBindTexture (GL_TEXTURE_2D, value->texture_id);

		glTexSubImage2D (GL_TEXTURE_2D, 0, r.x, r.y, r.width, r.height, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, r.data);
...
static void
add_to_cache (GskGLGlyphCache* self, GlyphCacheKey* key, GskGLCachedGlyph* value)
{
	...
	if (width < MAX_GLYPH_SIZE && height < MAX_GLYPH_SIZE) {
		...
		value->atlas = NULL;
		value->texture_id = driver_create_texture(width, height);
		driver_mark_texture_permanent(value->texture_id);

		driver_bind_source_texture(value->texture_id);
		driver_init_texture_empty(value->texture_id, GL_LINEAR, GL_LINEAR);
...
void
gsk_gl_glyph_cache_lookup_or_add (GskGLGlyphCache* cache, GlyphCacheKey* lookup, /*GskGLDriver* driver, */const GskGLCachedGlyph **cached_glyph_out)
{
	GskGLCachedGlyph* value = g_hash_table_lookup (cache->hash_table, lookup);
...

	{
		...
		value = AGL_NEW (GskGLCachedGlyph,
			.draw_x = ink_rect.x,
		);
...
void
gsk_gl_glyph_cache_begin_frame (GskGLGlyphCache* self, GPtrArray* removed_atlases)
{
	GHashTableIter iter;
	GlyphCacheKey* key;
	...
	if (self->timestamp % MAX_FRAME_AGE == 30) {
		...
		while (g_hash_table_iter_next (&iter, (gpointer*)&key, (gpointer*)&value)) {
			if (!value->accessed) {
				if (value->atlas) {
					...
					driver_destroy_texture (value->texture_id);
					g_hash_table_iter_remove (&iter);
		...
		dbg2(GLYPH_CACHE, 1, "%d glyphs cached", g_hash_table_size (self->hash_table));
	}

	if (dropped > 0) dbg2(GLYPH_CACHE, 1, "Dropped %d glyphs", dropped);
}
